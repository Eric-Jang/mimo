								...
								<tr>
									<td><c:choose>
											<c:when test="${bean.directory }">
												<a
													href="${ctx }/resource/list/?pathname=${codec:urlEncode(bean.path)}">
													[目录]&nbsp;${bean.name}</a>
											</c:when>
											<c:otherwise>
		[普通文件]&nbsp;${bean.name}
									...
									<td>&nbsp; <shiro:hasPermission name="resource:delete">
											<a href="javascript:void(0);" id="${bean.path }" class="del">删除</a>&nbsp;
		</shiro:hasPermission> <shiro:hasPermission name="resource:download">
											<a
												href="${ctx }/resource/download/?pathname=${codec:urlEncode(bean.path)}">下载</a>
										</shiro:hasPermission> &nbsp;
									</td>
								</tr>
							</c:forEach>
